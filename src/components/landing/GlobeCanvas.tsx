...
export default function GlobeCanvas({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      type Ping = { dot: Mesh; ring: Mesh; start: number; ttl: number };
      const pings: Ping[] = [];
      ...
      function loop() {
        ...
        for (let i = pings.length - 1; i >= 0; i--) {
          ...
          if (age >= 1) {
            ...
            (p.dot.material as Material).dispose();
            (p.ring.material as Material).dispose();
            pings.splice(i, 1);
            continue;
          }
          const s = 1 + age * 5.5;
          p.ring.scale.set(s, s, s);
          (p.ring.material as MeshBasicMaterial).opacity = 0.9 * (1 - age);
          (p.dot.material as MeshBasicMaterial).opacity = 1 - age * 0.4;
        }
